...
async function runSeed() {
  ...
  try {
    ...
    // Check & Seed Farmers Table
    const checkFarmers = await client.query('SELECT COUNT(*) FROM farmers');
    const farmerCount = parseInt(checkFarmers.rows[0].count);
    ...
    if (farmerCount === 0) {
      ...
      for (const f of DEFAULT_FARMERS) {
        await client.query(`
          INSERT INTO farmers (name, age, district, village, crop, disaster, goal, raised, land_area, damage, image, story, breakdown, gallery, timeline, verified, farmer_phone, requested_amount, video_proof, image_proofs)
        ...
        `, [
          ...
          "9876543210", f.goal, null, JSON.stringify([])
        ]);
      }
      console.log("Farmers seeded successfully!");
    } else {
      console.log("Farmers table already has data. Skipping seeding for farmers.");
    }

    // Fetch seeded/existing farmers to link donations
    const farmersResult = await client.query('SELECT id, name FROM farmers');

    // Check & Seed Donations Table
    const checkDonations = await client.query('SELECT COUNT(*) FROM donations');
    const donationCount = parseInt(checkDonations.rows[0].count);
    ...
    if (donationCount === 0) {
      ...
      console.log("Donations table already has data. Skipping seeding for donations.");
    }
